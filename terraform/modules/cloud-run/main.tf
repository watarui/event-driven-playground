...
# すべてのサービスでヘルスチェックエンドポイントを公開アクセス可能にする
# Cloud Run は URL パスベースの認証をサポートしていないため、サービス全体が公開される
# 将来的に API Gateway や Load Balancer を使用して細かい制御を実装予定
