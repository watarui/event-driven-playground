...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (isProduction && !initialAdminEmail) {
			console.log(
				"[init-admin] ERROR: INITIAL_ADMIN_EMAIL not set in production",
			);
			return NextResponse.json(
				{
					success: false,
					message:
						"Initial admin email not configured for production environment",
				},
				{ status: 500 },
			);
		}
		...
		const adminExists = allUsers.users.some(
			...
			return NextResponse.json(
				{
					success: false,
					message:
						"Admin already exists. Please contact existing admin to grant permissions.",
				},
				{ status: 403 },
			);
		}
