		...
		const updateHealth = async () => {
			...
			if (metrics) {
				...
				metrics.health.checks.forEach((check) => {
					...
					if (nodeId) {
						health[nodeId] = {
							...
							memoryUsage:
								nodeId === "client" ? metrics.memoryInfo.total_mb : undefined,
							processCount:
								nodeId === "client"
									? metrics.memoryInfo.process_count
									: undefined,
						};
					}
				});
				...
				nodes.forEach((node) => {
					...
					health[node.id] = {
						status:
							random > 0.9 ? "error" : random > 0.8 ? "warning" : "healthy",
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold text-green-600">
							{
								Object.values(serviceHealth).filter(
									(h) => h.status === "healthy",
								).length
							}
						</div>
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold text-red-600">
							{
								Object.values(serviceHealth).filter((h) => h.status === "error")
									.length
							}
						</div>
					</CardContent>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{Object.values(serviceHealth).length > 0
								? Math.round(
										Object.values(serviceHealth).reduce(
											(sum, h) => sum + h.latency,
											0,
